...
Skill text fields (`procedure`, `failure_recovery`, `prerequisites`) are treated as **optimizable text components** — a GEPA `Candidate` (`dict[str, str]`, confirmed against `gepa-ai/gepa`'s source) — evolved by [GEPA](https://github.com/gepa-ai/gepa) rather than by gradient updates. This is the direct translation of "weight-free" into an existing, sample-efficient method built for exactly this: reflective mutation of text components from textual + scalar feedback, with Pareto-frontier candidate selection across multiple objectives. Note this is the standalone `gepa` package's `GEPAAdapter`/`optimize()` API, not `dspy.GEPA` — the latter optimizes a `dspy.Module`'s predictors directly, which doesn't fit a skill's text living outside any DSPy program (see §6, and `IMPLEMENTATION_PLAN.md` §5 for the concrete adapter design).

**Feedback conversion:** a `feedback.py` module normalizes heterogeneous signals — compiler errors, failed test names/messages, tool `is_error` outputs, evaluator free-text comments — into the `(score, feedback_text)` pairs GEPA's reflection step consumes. This is the concrete answer to "convert heterogeneous execution feedback into controlled textual revisions": GEPA's reflective LM proposes new candidate component text conditioned on `feedback_text` for a batch of recent evaluations, not on scores alone.

**Held-out evaluation is native, not bolted on:** `gepa.optimize()` takes `trainset` and `valset` as separate arguments and tracks each candidate's aggregate score on `valset` throughout the run — P4's in-domain held-out check (§5.4) can be satisfied directly by this split for the batch GEPA itself explored, rather than requiring a wholly separate re-evaluation pass. `optimize()` also exposes `max_metric_calls`/`max_reflection_cost` as native budget caps, which is where the cost control this section needs actually belongs, rather than a downstream cost audit.

**Drift/contradiction controls (GEPA gives sample efficiency, not safety — these are added on top, applied to `GEPAResult.best_candidate` after the run):**
