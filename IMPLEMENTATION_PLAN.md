...
- End-to-end demonstration on one skill from the pinned set: inject v1, run `gepa_runner.py` with a real train/val split of that skill's covered tasks, get back a `GEPAResult` whose best candidate's `val_aggregate_scores` beats v1's on `valset`, pass it through `promotion_gate.py`. Because `valset` is scored by GEPA itself throughout the run (not just at the end), this result is a straightforward read of `GEPAResult`, not a separate re-run we have to construct.
